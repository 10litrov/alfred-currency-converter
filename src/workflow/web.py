...
class Response(object):
    ...
    def xml(self):
        """Decode response contents as XML.

        :returns: object decoded from XML
        :rtype: :class:`dict`

        """

        def convert_xml_to_dict(node):
            result = {}
            ...
            for child in node:
                new_item = convert_xml_to_dict(child)
                tag = child.tag
                if result.has_key(tag):
                    # convert duplicate tags to list of values
                    if type(result[tag]) == list:
                        result[tag].append(new_item)
                    else:
                        result[tag] = [result[tag], new_item]
                else:
                    result[tag] = new_item

            text = node.text.strip() if node.text is not None else ''
            # set the text as a dictionary value
            # or set the result to text if there are no child nodes or attributes
            if len(result):
                if len(text):
                    result['_text'] = text
            else:
        ...
        return convert_xml_to_dict(ElementTree.fromstring(self.content))
